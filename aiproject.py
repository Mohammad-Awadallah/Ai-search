...
def optimalway(s, g):
    ...
    while (p[1] not in go):
        ...
        for i in range(len(cost[p[1]])):
            if cost[p[1]][i] > 0 and (listofnodes[ind].city_id not in visited):#hcost + real cost
...
def greedy(s, g):
    ...
    while (p[1] not in go):
        ...
        for i in range(len(hcost[p[1]])):
            if hcost[p[1]][i] > 0 and (listofnodes[ind].city_id not in visited):#hcost
                fringe.append(
...
def ucl(s, g):
    ...
    while (p[1] not in go):
        ...
        ind = 0
        for i in cost[p[1]]:
            if i > 0 and (listofnodes[ind].city_id not in visited):#real cost
